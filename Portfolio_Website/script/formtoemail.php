...
$my_email = "C0504554@online.camosun.ca";
...
$from_email = "C0504554@online.camosun.ca";
...
$continue = "https://student.camosun.bc.ca/MauricioV/project/";
...
Gobbledegook check.  You sometimes get spam consisting entirely of gobbledegook characters (Å ð ç etc).  These occur when a spammer takes a guess at encoding and gets it wrong.  This check will prevent any messages getting through if gobbledegook is found.  By default this check is not done.  The default value is: $gobbledegook_check = 0;  To enable it, change the value to 1, like so: $gobbledegook_check = 1;
...
<?php
require_once('file:///Y|/Beninger/ABT294/D20_StartFiles/recaptchalib.php');
...
Note that you have to enter your reCAPTCHA public key in the above code (same too if using the sample form code above).  Your form page MUST be a PHP page for the reCAPTCHA code to work.  i.e. your page must have a .php extension like "contact.php" for example.  You will also need to download the PHP zip file from recaptcha.net and put the file "file:///Y|/Beninger/ABT294/D20_StartFiles/recaptchalib.php" in the same directory/folder as the form page and script.  The zip file from recaptcha.net has several files in it but "file:///Y|/Beninger/ABT294/D20_StartFiles/recaptchalib.php" is the only one you need.  You can get it here: http://code.google.com/p/recaptcha/downloads/list?q=label:phplib-Latest
...
textCAPTCHA system.  textCAPTCHA is a third-party captcha system (more information here: http://textcaptcha.com/).  As the name implies, it is a text-based captcha system, presenting security questions in plain text, which the visitor must answer correctly to submit the form.  This is particularly useful for people who are visually impaired.  It presents logic questions like (for example): "The colour of a pink cake is?".

To use this captcha system on your form, you need to do the following:

1. Register with textCAPTCHA to get a unique API key.
2. Place some PHP code (including your unique API key) on your form to display the textCAPTCHA.
3. Enable the $textCAPTCHA option below.

In more detail:

1. To use textCAPTCHA you need a unique API key.  You get this by registering with them, here: http://textcaptcha.com/register  They will send your unique API key by email.

2. Place some PHP code on your form page to display the textCAPTCHA.  Your form page needs to be a PHP page with a .php extension, like "contact.php" for example.  If your form page has a .htm or .html extension, simply save it with a .php extension.  This system uses a session, so you need to have support for sessions on your PHP setup (you will have this by default).  You also need to have allow_url_fopen enabled, which you should also have by default.

Place this PHP code (between the asterisks) on your form page AT THE VERY TOP and ADD YOUR UNIQUE API KEY TO IT:
...
$my_api_key = "enter your unique API key in here";

$textcaptcha_url = "http://textcaptcha.com/api/";

$textcaptcha = file_get_contents($textcaptcha_url.$my_api_key);

$textcaptcha = explode("</question>",$textcaptcha);

$textcaptcha_question = explode("<question>",$textcaptcha[0]);

$textcaptcha_question = str_replace("&apos;","'",$textcaptcha_question[1]);

$textcaptcha_answer = explode("</answer>",$textcaptcha[1]);

$answer = array();

foreach($textcaptcha_answer as $value)
{

$textcaptcha_answer2 = explode("<answer>",$value);

if(isset($textcaptcha_answer2[1])){$answer[] = $textcaptcha_answer2[1];}

}

$_SESSION['textcaptcha_answer'] = $answer;

?>
***

In the above code, enter your unique API key, like this example:

$my_api_key = "pb1sqjtpw1wgkk8808wc0osk8755pdjd";

Then, to show the security question and a text input for the visitor's answer on your form, use this code:

<?php print $textcaptcha_question ?><br><input type="text" size="30" name="textcaptcha">

...place the above line of code within your form code, between the <form action="formtoemailpro.php" method="post"></form> tags.

Here's a sample form using the above line of code:

<form action="formtoemailpro.php" method="post">
<table width="350" border="0" style="background:#ececec" cellspacing="5">
<tr align="left"><td>Name</td><td><input type="text" size="30" name="name"></td></tr>
<tr align="left"><td>Email</td><td><input type="text" size="30" name="email"></td></tr>
<tr align="left"><td valign="top">Comments</td><td><textarea name="comments" rows="6" cols="30"></textarea></td></tr>
<tr align="left"><td>&nbsp;</td><td><?php print $textcaptcha_question ?><br><input type="text" size="30" name="textcaptcha"></td></tr>
<tr align="left"><td>&nbsp;</td><td><input type="submit" value="Send"></td></tr>
</table>
</form>

Save your form page and upload it to your webspace.

3. By default this option is not enabled ($textCAPTCHA = 0;).  To enable it, change the 0 below to 1, like so: $textCAPTCHA = 1;
...
$path_to_csv_file = dirname(__FILE__) . "e:/SofB_FTP/C9999998/nomadFS/broch/";
...
if(!isset($_REQUEST['textcaptcha']) || empty($_REQUEST['textcaptcha']) || !in_array(md5(strtolower(trim($_REQUEST['textcaptcha']))),$_SESSION['textcaptcha_answer'])){$errors[] = "Security answer is incorrect";}else{unset($_REQUEST['textcaptcha']);}
...
foreach($required_fields as $value){if((!isset($_REQUEST[$value]) || (empty($_REQUEST[$value]) && $_REQUEST[$value] != "0")) && (!isset($_FILES[$value]['name']) || empty($_FILES[$value]['name']))){$errors[] = "Please complete the $value field";}}
...
if($encode_name_subject){$autoresponder_subject = "=?{$character_set}?B?".base64_encode($autoresponder_subject)."?=";}
